...
def formation_of_set_S_vertical(list_bottom, S1, list_shape1):  # get list of shapes which are already placed below of given shape
    list_shape3 = []
    ...
    for i in range(len(S1)):
        S2 = []
        if intersection_of_shapes(list_bottom, S1[i]) == True and intersection_of_shapes(list_shape1, S1[i]) == False:
            if S1[i] != list_shape1:
                S.append(S1[i])
